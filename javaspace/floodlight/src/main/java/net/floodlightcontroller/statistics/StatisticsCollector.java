...
public class StatisticsCollector implements IFloodlightModule, IStatisticsService {
	 ...
	 *
	 */
	private class PortStatsCollector implements Runnable {  //实现多线程，要实现Runnable接口的run方法

		@Override
		public void run() {   //要实现多线程，要实现Runnable接口的run方法
			// run方法的主体在处理端口回复消息 replies
			...
			for (Entry<DatapathId, List<OFStatsReply>> e : replies.entrySet()) {
				for (OFStatsReply r : e.getValue()) {
					...
					for (OFPortStatsEntry pse : psr.getEntries()) {
						...
						// portStats 是map，key为NodePortTuple, value为SwitchPortBandwidth
						if (portStats.containsKey(npt) || tentativePortStats.containsKey(npt)) {
							...
							if (spb.getPriorByteValueRx().compareTo(pse.getRxBytes()) > 0) { /* overflow */
								U64 upper = U64.NO_MASK.subtract(spb.getPriorByteValueRx()); //NO_MASK 0xFFffFFffFFffFFffL;
								U64 lower = pse.getRxBytes();
								rxBytesCounted = upper.add(lower);
							} else {
								rxBytesCounted = pse.getRxBytes().subtract(spb.getPriorByteValueRx());// 减去
							}
							...
							// jian 2016 -5 -20
							/*
							log.info(" switch {}  port {} ",spb.getSwitchId(),spb.getSwitchPort());
							log.info("Counted Byte  Tx  {}    Rx is {} ",txBytesCounted.getValue(),rxBytesCounted.getValue());										
							log.info(" time is {}",timeDifSec);
							*/
			...
			for(NodePortTuple npt:portStats.keySet()){	
				...
				{ log.info("Bit/s    Rx : {}    Tx : {} ",portStats.get(npt).getBitsPerSecondRx().getValue(),portStats.get(npt).getBitsPerSecondTx().getValue());
			...
			for(NodePortTuple npt:tentativePortStats.keySet()){	
				log.info(" tentativePortStats switch and port {}",npt);
				log.info("Bit/s  Tx  {}    Rx is {} ",tentativePortStats.get(npt).getBitsPerSecondTx().getValue(),tentativePortStats.get(npt).getBitsPerSecondRx().getValue());
				log.info("Byte  Tx  {}    Rx is {} ",tentativePortStats.get(npt).getPriorByteValueTx().getValue(),tentativePortStats.get(npt).getPriorByteValueRx().getValue());
			}	
			*/
			
		}  // end of run()
			
	}  // end of class PortStatsCollector
	...
	private class GetStatisticsThread extends Thread {
		...
		@Override
		public void run() {
			statsReply = getSwitchStatistics(switchId, statType); //调用的是后一个 重载函数
		}
	}
	...
	/*
	 * IStatisticsService implementation   IStatisticsService服务实现
	 */
	...
	@Override
	public synchronized void collectStatistics(boolean collect) { //在ConfigResource.java中 config函数中调用
	...
	private void startStatisticsCollection() {
		// 通过线程池服务启动 portStatsCollector 这个线程
		portStatsCollector = threadPoolService.getScheduledExecutor().scheduleAtFixedRate(
				new PortStatsCollector(), portStatsInterval, portStatsInterval, TimeUnit.SECONDS);
		// 端口带宽数据清空
	...
	private Map<DatapathId, List<OFStatsReply>> getSwitchStatistics(Set<DatapathId> dpids, OFStatsType statsType) {
		...
		// 创建交换机数量 的线程数   GetStatisticsThread 是继承自Thread 的线程
		...
		/*Join all the threads after the timeout. Set a hard timeout of 12 seconds for the threads to finish. If the thread has not
		 * finished the switch has not replied yet and therefore we won't add the switch's stats to the reply.*/
		//如果在间隔时间内，线程结束，则将交换机的回复数据加到map里面
	...
	// 重载函数 getSwitchStatistics( )可以处理更多类型的数据请求消息
